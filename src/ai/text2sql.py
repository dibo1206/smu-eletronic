...
# 학생들이 실제로 쓰는 학과 줄임말 — DB에는 정식 학과명만 있으므로, SQL을
# 생성할 때 이 매핑을 보고 정식 학과명으로 바꿔 조회하도록 프롬프트에 알려준다.
DEPARTMENT_ALIASES = {
    "휴먼지능로봇공학과": ["휴지로"],
    "소프트웨어학과": ["솦웨"],
    "시스템반도체공학과": ["시반공"],
...
질문: 휴먼지능로봇공학과 전공 알려줘
SQL: SELECT 구분, 항목 FROM 진로정보 WHERE 학과명 = '휴먼지능로봇공학과' ORDER BY 구분, 순번 LIMIT 30;

질문: 휴지로 2학년 전공과목 알려줘
SQL: SELECT 과목명, 담당교수, 요일, 시작시간, 종료시간 FROM 강좌 WHERE 학과 = '휴먼지능로봇공학과' AND 학년 = 2 AND 이수구분 LIKE '전공%' LIMIT 20;
</examples>
"""
...
질문에 줄임말이 나오면 그에 해당하는 정식 학과명으로 바꿔서 학과/학과명
컬럼을 조회하세요 (DB에는 정식 학과명만 저장되어 있습니다).
</department_aliases>

<rules>
- SQLite 문법을 사용하세요
- SELECT 쿼리, 또는 WITH로 시작하는 CTE(SELECT로 끝나는)를 생성하세요 (INSERT, UPDATE, DELETE 금지)
- 결과는 최대 20개로 제한하세요 (LIMIT 20). 단, 진로정보에서 구분을 좁히지
  않고 학과 전체를 가져오는 경우에는 한 학과당 최대 25건이라 잘리지 않게
  LIMIT 30을 쓰세요.
...
- 진로정보를 물을 때 취업분야/진출직업/관련자격증 중 어느 것인지 질문에
  명시돼 있지 않으면(예: "OO학과 전공 알려줘"처럼 학과명만 있는 경우),
  구분으로 좁히지 말고 학과의 모든 구분(취업분야/진출직업/관련자격증/비고)을
  구분과 함께 전부 가져오세요 — 하나만 임의로 골라서 답하면 안 됩니다.
